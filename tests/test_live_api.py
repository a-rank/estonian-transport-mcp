...
class TestPlanTripLive:
    ...
    async def test_tallinn_short_trip(self):
        ...
        result = await plan_trip("Viru", "Balti jaam")
        assert "Option 1" in result or "No routes found" in result

    @pytest.mark.asyncio
    async def test_tallinn_to_tartu(self):
        """Longer trip — Tallinn to Tartu."""
        result = await plan_trip("Viru", "Tartu")
        assert "Option 1" in result or "No routes found" in result

    @pytest.mark.asyncio
    async def test_unknown_place(self):
        """Unknown place name should return a helpful message."""
        result = await plan_trip("xyznonexistent12345", "Viru")
        assert "Could not find" in result

    @pytest.mark.asyncio
    async def test_with_date_and_time(self):
        """Verify date and time parameters are accepted."""
        result = await plan_trip("Viru", "Balti jaam", date="2026-04-01", time="09:00")
        assert "Option 1" in result or "No routes found" in result

    @pytest.mark.asyncio
    async def test_arrive_by(self):
        """Test arrive_by parameter."""
        result = await plan_trip("Viru", "Balti jaam", time="10:00", arrive_by=True)
        assert "Option 1" in result or "No routes found" in result
